...
def consultaRegistro(ventas, idProducto):
    ...
    for venta in ventas:
        if venta[0] == idProducto:
            lista.append(venta)
            print(
                f'Producto consultado : {idProducto}  Descripción  {venta[1]}  #Parte  {venta[2]}  Cantidad vendida  {venta[3]}  Stock  {venta[4]}  Comprador {venta[5]}  Documento  {venta[6]}  Fecha Venta  {venta[7]}')
    if len(lista) == 0:
